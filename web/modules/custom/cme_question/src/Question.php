...
class Question {
   ...
   * @return Mixed
   */
  public function getQuestionArchived($question) {
    switch ($question->get('field_question_type')->value) {
      case 'true_false':
        $output = $this->trueFalseArchived($question);
        break;
      case 'single':
        $output = $this->singleChoiceArchived($question);
        break;
      case 'multiple':
        $output = $this->multipleChoiceArchived($question);
        break;
      default:
        $output = FALSE;
    }
   ...
   * Single
   */
  public function singleChoiceArchived($question) {
    $output = '';
    foreach ($question->get('field_single_choice')->getValue() as $single) {
      $para = \Drupal\paragraphs\Entity\Paragraph::load($single['target_id']);
      $answerd = '';
      if ($para->get('field_correct_answer')->value == 1) {
        $answerd = 'checked="checked"';
      }
      $output .= ' <label display="inline-flex" class="bjjYsL">';
      $output .= '<span class="gSrgng">';
      $output .= '<input disabled="disabled" ' . $answerd . ' type="radio" id="single-' . $para->id() . '" class="psDju" name="single_' . $question->id() . '" value="' . $para->id() . '">';
      $output .= '<span class="gsdkyl"></span>';
      $output .= '</span>';
      $output .= '<span class="fVCIOT"></span><span class="hSMymM">' . $para->get('field_answer')->value . '</span></span>';
      $output .= '</label>';
   ...
   * Single
   */
  public function multipleChoiceArchived($question) {
    $output = '';
    foreach ($question->get('field_multiple_choice')->getValue() as $multiple) {
      $para = \Drupal\paragraphs\Entity\Paragraph::load($multiple['target_id']);
      $answerd = '';
      ...
      $output .= ' <label display="inline-flex" class="bjjYsL">';
      $output .= '<span class="gSrgng">';
      $output .= '<input ' . $answerd . ' disabled="disabled" type="checkbox" id="multiple-' . $para->id() . '" class="psDju" name="multiple_' . $question->id() . '[]" value="' . $para->id() . '">';
      $output .= '<span class="gsdkyl"></span>';
      $output .= '</span>';
      $output .= '<span class="fVCIOT"></span><span class="hSMymM">' . $para->get('field_answer')->value . '</span></span>';
      $output .= '</label>';
   ...
   * @return string
   */
  public function trueFalseArchived($question) {
    $para = \Drupal\paragraphs\Entity\Paragraph::load($question->get('field_true_false')->target_id);
    $true = '';
    if($para->get('field_true_false')->value == 'True'){
      $true = 'checked="checked"';
    }
    $false = '';
    if($para->get('field_true_false')->value == 'False'){
      $false = 'checked="checked"';
    }
    $output = '';
    $output .= ' <label display="inline-flex" class="bjjYsL">';
    $output .= '<span class="gSrgng">';
    $output .= '<input '.$true.' disabled type="radio" id="true-' . $question->id() . '" class="psDju" name="truefalse_' . $question->id() . '" value="true_' . $question->id() . '">';
    $output .= '<span class="gsdkyl"></span>';
    $output .= '</span>';
    $output .= '<span class="fVCIOT"></span><span class="hSMymM">True</span></span>';
    $output .= '</label>';
    ...
    $output .= ' <label display="inline-flex" class="bjjYsL">';
    $output .= '<span class="gSrgng">';
    $output .= '<input '.$false.' disabled type="radio" id="false-' . $question->id() . '" class="psDju" name="truefalse_' . $question->id() . '" value="false_' . $question->id() . '">';
    $output .= '<span class="gsdkyl"></span>';
    $output .= '</span>';
    $output .= '<span class="fVCIOT"></span><span class="hSMymM">False</span></span>';
    $output .= '</label>';

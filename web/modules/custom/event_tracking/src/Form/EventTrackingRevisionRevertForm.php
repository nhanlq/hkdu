...
/**
 * Provides a form for reverting a Event tracking revision.
 *
 * @ingroup event_tracking
 */
class EventTrackingRevisionRevertForm extends ConfirmFormBase {
  ...
  public function getFormId() {
    return 'event_tracking_revision_revert_confirm';
  }
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    $this->logger('content')->notice('Event tracking: reverted %title revision %revision.', ['%title' => $this->revision->label(), '%revision' => $this->revision->getRevisionId()]);
    $this->messenger()->addMessage(t('Event tracking %title has been reverted to the revision from %revision-date.', ['%title' => $this->revision->label(), '%revision-date' => $this->dateFormatter->format($original_revision_timestamp)]));
   ...
   *
   * @param \Drupal\event_tracking\Entity\EventTrackingInterface $revision
   ...
   */
  protected function prepareRevertedRevision(EventTrackingInterface $revision, FormStateInterface $form_state) {

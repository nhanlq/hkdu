...
/**
 * Provides a form for reverting a Banner entity revision.
 *
 * @ingroup banner
 */
class BannerEntityRevisionRevertForm extends ConfirmFormBase {
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    $this->logger('content')->notice('Banner entity: reverted %title revision %revision.', ['%title' => $this->revision->label(), '%revision' => $this->revision->getRevisionId()]);
    $this->messenger()->addMessage(t('Banner entity %title has been reverted to the revision from %revision-date.', ['%title' => $this->revision->label(), '%revision-date' => $this->dateFormatter->format($original_revision_timestamp)]));
   ...
   *
   * @param \Drupal\banner\Entity\BannerEntityInterface $revision
   ...
   */
  protected function prepareRevertedRevision(BannerEntityInterface $revision, FormStateInterface $form_state) {

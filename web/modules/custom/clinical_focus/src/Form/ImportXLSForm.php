...
class ImportXLSForm extends FormBase
{
    ...
    public function buildForm(array $form, FormStateInterface $form_state)
    {
        $form['excel'] = [
            ...
            '#description' => $this->t('Allow Extension: xls, xlsx. Click <a target="_blank" href="/sites/default/files/import/example.xlsx">here</a> to download example.')
        ];
        $form['submit'] = [
            '#type' => 'submit',
            '#value' => $this->t('Submit'),
...
//    }
        ...
        foreach( $sheetData as $data){
            if($i != 1){
                ...
                $clinical_focus = \Drupal\clinical_focus\Entity\ClinicalFocus::create([
                    'name' => $data['A'],
                    'uid' => $user->id(),
                    'field_image'=>$file->id(),
                    'field_description' => ['value' => $data['C'], 'format' => 'full_html'],
                    'status' => $data['E'],
                    'field_is_home' => $data['F'],
                    'field_weight' => $data['G'],
                    'field_tags' => $this->getTagsTid($data['D'])
                ]);
                try {
                    $clinical_focus->save();

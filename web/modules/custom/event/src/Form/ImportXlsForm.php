   ...
   */
    public function buildForm(array $form, FormStateInterface $form_state)
    {
        $form['excel'] = [
            ...
            '#description' => $this->t('Allow Extension: xls, xlsx. Click <a target="_blank" href="/sites/default/files/import/event_example.xlsx">here</a> to download example.')
        ];
        $form['submit'] = [
            '#type' => 'submit',
            '#value' => $this->t('Submit'),
...
//    }
        ...
        foreach( $sheetData as $data){
            if($i != 1){
                ...
                $price = new \Drupal\commerce_price\Price($data['G'], 'HKD');
                $date = date('Y-m-d', strtotime($data['D']));
                $event = \Drupal\event\Entity\Event::create([
                    'name' => $data['A'],
                    'uid' => $user->id(),
                    'field_image'=>$file->id(),
                    ...
                    'field_date' => $date,
                    'field_time' => $data['E'],
                    'field_free_price_text' => $data['F'],
                    'field_price' => $price,
                    'field_location' => $data['H'],
                    'status' => $data['J'],
                    'field_is_home' => $data['K'],
                    'field_weight' => $data['L'],
                    'field_tags' => $this->getTagsTid($data['I'])
                ]);
                try {
                    $event->save();

...
class EventController extends ControllerBase implements ContainerInjectionInterface {
  ...
  public function revisionShow($event_revision) {
    $event = $this->entityTypeManager()->getStorage('event')
      ->loadRevision($event_revision);
    $view_builder = $this->entityTypeManager()->getViewBuilder('event');
  ...
  public function revisionPageTitle($event_revision) {
    ...
    return $this->t('Revision of %title from %date', [
      '%title' => $event->label(),
      '%date' => $this->dateFormatter->format($event->getRevisionCreationTime()),
    ]);
  }
    ...
    foreach (array_reverse($vids) as $vid) {
      ...
      if ($revision->hasTranslation($langcode) && $revision->getTranslation($langcode)->isRevisionTranslationAffected()) {
        ...
        if ($vid != $event->getRevisionId()) {
          $link = $this->l($date, new Url('entity.event.revision', [
            'event' => $event->id(),
            'event_revision' => $vid,
          ]));
        }

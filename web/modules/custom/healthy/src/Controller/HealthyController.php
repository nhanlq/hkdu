...
class HealthyController extends ControllerBase implements ContainerInjectionInterface {
  ...
  public function revisionShow($news_revision) {
    $news = $this->entityTypeManager()->getStorage('healthy')->loadRevision($news_revision);
  ...
  public function revisionOverview(HealthyInterface $news) {
    ...
    $build['#title'] = $has_translations ? $this->t('@langname revisions for %title',
      ['@langname' => $langname, '%title' => $news->label()]) : $this->t('Revisions for %title',
      ['%title' => $news->label()]);
      ...
      if ($revision->hasTranslation($langcode) && $revision->getTranslation($langcode)
        ...
        if ($latest_revision) {
          ...
          if ($revert_permission) {
            $links['revert'] = [
              'title' => $this->t('Revert'),
              'url' => $has_translations ? Url::fromRoute('entity.healthy.translation_revert', [
                'healthy' => $news->id(),
                'healthy_revision' => $vid,
                'langcode' => $langcode,
  ...
  /**
   * @return \Drupal\Core\Entity\EntityBase[]|\Drupal\Core\Entity\EntityInterface[]|\Drupal\healthy\Entity\Healthy[]
  ...
  public function getTags() {
    $tags = [];
    $ids = \Drupal::entityQuery('news')->condition('status', 1)->execute();
    $result = Healthy::loadMultiple($ids);
  ...
  public function title() {
    return [
      '#markup' => \Drupal::state()->get('/healthy', 'Public Healthy'),

...
/**
 * Provides a form for reverting a Download revision.
 *
 * @ingroup download
 */
class DownloadRevisionRevertForm extends ConfirmFormBase {
  ...
  public function getFormId() {
    return 'download_revision_revert_confirm';
  }
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    $this->logger('content')->notice('Download: reverted %title revision %revision.', ['%title' => $this->revision->label(), '%revision' => $this->revision->getRevisionId()]);
    $this->messenger()->addMessage(t('Download %title has been reverted to the revision from %revision-date.', ['%title' => $this->revision->label(), '%revision-date' => $this->dateFormatter->format($original_revision_timestamp)]));
   ...
   *
   * @param \Drupal\download\Entity\DownloadInterface $revision

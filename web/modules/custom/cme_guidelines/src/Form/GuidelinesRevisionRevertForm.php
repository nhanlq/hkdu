...
/**
 * Provides a form for reverting a Guidelines revision.
 *
 * @ingroup cme_guidelines
 */
class GuidelinesRevisionRevertForm extends ConfirmFormBase {
  ...
  public function getFormId() {
    return 'guidelines_revision_revert_confirm';
  }
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    $this->logger('content')->notice('Guidelines: reverted %title revision %revision.', ['%title' => $this->revision->label(), '%revision' => $this->revision->getRevisionId()]);
    $this->messenger()->addMessage(t('Guidelines %title has been reverted to the revision from %revision-date.', ['%title' => $this->revision->label(), '%revision-date' => $this->dateFormatter->format($original_revision_timestamp)]));
   ...
   *
   * @param \Drupal\cme_guidelines\Entity\GuidelinesInterface $revision
   ...
   */
  protected function prepareRevertedRevision(GuidelinesInterface $revision, FormStateInterface $form_state) {
